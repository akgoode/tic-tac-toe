...
/***/ function(module, exports, __webpack_require__) {
	...
	var gameStore = __webpack_require__(13);
	...
	Board.prototype.isHorizontalWin = function (piece) {
	  var win1 = this.spaces[0] === piece && this.spaces[1] === piece && this.spaces[2] === piece;
	  var win2 = this.spaces[3] === piece && this.spaces[4] === piece && this.spaces[5] === piece;
	  var win3 = this.spaces[6] === piece && this.spaces[7] === piece && this.spaces[8] === piece;
	  return win1 || win2 || win3;
	};

	Board.prototype.isVerticalWin = function (piece) {
	  var win1 = this.spaces[0] === piece && this.spaces[3] === piece && this.spaces[6] === piece;
	  var win2 = this.spaces[1] === piece && this.spaces[4] === piece && this.spaces[7] === piece;
	  var win3 = this.spaces[2] === piece && this.spaces[5] === piece && this.spaces[8] === piece;
	  return win1 || win2 || win3;
	};

	Board.prototype.isDiagonalWin = function (piece) {
	  var win1 = this.spaces[0] === piece && this.spaces[4] === piece && this.spaces[8] === piece;
	  var win2 = this.spaces[6] === piece && this.spaces[4] === piece && this.spaces[2] === piece;
	  return win1 || win2;
	};

	var createBoard = function createBoard(board1, gameboard) {
	  for (var i = 0; i < board1.spaces.length; i++) {
	    gameboard.append('<div class="space" id="' + i + '"></div>');
	    $('#' + i).append('<h1 id="text' + i + '"></h1>');
	    var bindMakeMove = makeMove.bind(board1);
	...
	function boardInit() {
	  var $board = $('#gameboard');
	  var board1 = new Board(gameStore.game);
	...
	function getUserWins(data) {
	  ...
	  for (var i = 0; i < games.length; i++) {
	    var game = new Board(games[i]);
	    var numX = 0;
	    var numO = 0;
	    for (var j = 0; j < game.spaces.length; j++) {
	      if (game.spaces[j] === 'x') {
	        numX++;
	...
	module.exports = {
	  ...
	  getUnfinishedGamesIds: getUnfinishedGamesIds
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ function(module, exports, __webpack_require__) {
	...
	var update = function update(move, board) {
	  return $.ajax({
...
/***/ function(module, exports, __webpack_require__) {
	...
	var onNewGame = function onNewGame(event) {
	  event.preventDefault();
	  gameAPI.createGame().then(function (response) {
	...
	var onShowGames = function onShowGames(event) {
	  event.preventDefault();
	  gameAPI.showGames().then(ui.showGamesSuccess).catch(ui.failure);
	};

	var onShowGame = function onShowGame(event) {
	  event.preventDefault();
	  var id = parseInt($('#game-id').val());
	  gameAPI.showGame(id).then(ui.showGameSuccess).catch(ui.showFailure);
	};

	var onUnfinishedGames = function onUnfinishedGames(event) {
	  event.preventDefault();
	  gameAPI.showGames().then(ui.unfinishedSuccess).catch(ui.failure);
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var unfinishedSuccess = function unfinishedSuccess(data) {
	  var games = board.getUnfinishedGamesIds(data);
	  if (games.length > 0) {
	...
	module.exports = {
	  ...
	  unfinishedSuccess: unfinishedSuccess,
	  failure: failure,
